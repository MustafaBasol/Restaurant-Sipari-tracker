...
export const TableHeaderCell: React.FC<{ children: ReactNode, align?: 'left' | 'right', isAction?: boolean }> = ({ children, align = 'left', isAction = false }) => (
    <th className={`px-6 py-3 text-${align} text-xs font-medium text-text-secondary uppercase tracking-wider ${isAction ? 'relative' : ''}`}>
        {children}
    </th>
);
...
export const TableCell: React.FC<{ children: ReactNode, align?: 'left' | 'right' }> = ({ children, align = 'left' }) => (
