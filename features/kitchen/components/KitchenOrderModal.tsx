    ...
    <Modal isOpen={!!order} onClose={onClose} title={t('kitchen.orderDetails')}>
      <div className="p-6">
        <div className="mb-4 bg-white p-4 rounded-xl shadow-subtle space-y-2">
          ...
          {table?.customerName && (
            <p className="text-lg font-bold">Customer: {table.customerName}</p>
          )}
          {table?.note && (
            <p className="text-base font-medium text-text-primary">Masa Notu: "{table.note}"</p>
          )}
          {order.note && (
            <div className="mt-2 bg-yellow-100 border border-yellow-300 p-3 rounded-lg">
              <p className="text-base font-bold text-yellow-900">
                {t('kitchen.generalOrderNote')}:
              </p>
        ...
        <div className="space-y-3">
          {order.items.map((item) => {
            ...
            const isToggleable = [OrderStatus.IN_PREPARATION, OrderStatus.READY].includes(
              item.status,
            );
            ...
            return (
              <div
                key={item.id}
                className={`bg-white p-4 rounded-xl shadow-subtle flex items-center justify-between ${item.status === OrderStatus.CANCELED ? 'opacity-60' : ''}`}
              >
                <div>
                  <p
                    className={`font-bold text-lg ${item.status === OrderStatus.CANCELED ? 'line-through' : ''}`}
                  >
                ...
                <div className="flex items-center gap-2">
                  {isFinalState ? (
                    <Badge variant={item.status === OrderStatus.CANCELED ? 'red' : 'gray'}>
