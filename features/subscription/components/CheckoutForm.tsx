    ...
    return (
        <form id="payment-form" onSubmit={handleSubmit}>
            ...
            <Button disabled={isLoading || !stripe || !elements} id="submit" className="w-full mt-6">
                <span id="button-text">
                    {isLoading ? t('subscription.checkout.processing') : t('subscription.checkout.payButton').replace('{amount}', '9.90 €')}
                </span>
            </Button>
            {message && <div id="payment-message" className="text-sm text-red-500 mt-4 text-center">{message}</div>}
        </form>
    );
};
