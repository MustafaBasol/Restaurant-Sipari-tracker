...
const TableGrid: React.FC<TableGridProps> = ({ onSelectTable }) => {
    ...
    const getTableStatusClass = (status: TableStatus) => {
        switch (status) {
            case TableStatus.FREE: return 'border-status-free';
            case TableStatus.OCCUPIED: return 'border-status-occupied';
            case TableStatus.CLOSED: return 'border-status-closed';
                ...
                return (
                     <div key={table.id} className="flex flex-col gap-2">
                        ...
                        >
                            ...
                            <span className="text-sm font-medium text-text-secondary">{t(`statuses.${table.status}`)}</span>
