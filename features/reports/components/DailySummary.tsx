...
const DateRangePresets: React.FC<{ onSelect: (start: string, end: string) => void }> = ({ onSelect }) => {
    const { t } = useLanguage();
...
const DailySummary: React.FC = () => {
    ...
    const formattedStartDate = formatDateTime(startDate + 'T00:00:00', timezone, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' });
    const formattedEndDate = formatDateTime(endDate + 'T00:00:00', timezone, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' });
            ...
            {isLoading ? (
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <StatCard title={t('reports.totalRevenue')} value={formatCurrency(data.totalRevenue, currency)} />
                        <StatCard title={t('reports.totalOrders')} value={data.totalOrders} />
                        <StatCard title={t('reports.averageTicket')} value={formatCurrency(data.averageTicket, currency)} />
                    </div>
