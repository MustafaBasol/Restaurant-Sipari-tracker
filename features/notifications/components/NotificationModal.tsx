...
const OrderDetails: React.FC<{ order: Order }> = ({ order }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-subtle mb-4">
            ...
            <p className="text-xs text-text-secondary mb-2">{new Date(order.createdAt).toLocaleTimeString()}</p>
    ...
    return (
        <Modal isOpen={isModalOpen} onClose={closeModal} title={t('notifications.title')}>
            <div className="p-6">
                {newOrders.length > 0 ? (
                    <div>
                        {newOrders.map(order => <OrderDetails key={order.id} order={order} />)}
                    </div>
                ) : (
                    <p className="text-center text-text-secondary">{t('notifications.noNewOrders')}</p>
                )}
                <div className="mt-6 flex justify-end">
                    <Button onClick={closeModal} variant="secondary">{t('notifications.dismiss')}</Button>
                </div>
            </div>
        </Modal>
    );
